# most part of the code is copy pasted from my another repo:
# https://github.com/scott-pu-pennstate/dktt_light

import logging
from argparse import ArgumentParser
import time
import os

import tensorflow as tf
import numpy as np
from sklearn.metrics import roc_auc_score

import preprocessing
import dpfa_config
import model as dpfa_model
from losses import PaddedBinaryCrossentropyLoss

__last_modified__ = time.time() - os.path.getmtime(__file__)


logger = logging.getLogger(__file__)
# todo: set level after parase user config
logger.setLevel(logging.INFO if dpfa_config.VERBOSE == 1 else logging.WARNING)
console = logging.StreamHandler()
console.setLevel(logging.INFO if dpfa_config.VERBOSE == 1 else logging.WARNING)
logger.addHandler(console)


def evaluate(test_inputs, test_y, model, test_size, batch_size=128):
    pt = 0
    test_probs = []

    while pt < test_size:
        batch_inputs = dict([(key, val[pt: pt + batch_size]) for key, val in test_inputs.items()])
        batch_probs = model(batch_inputs, training=False)
        batch_probs = batch_probs.numpy()

        test_probs.append(batch_probs)
        pt += batch_size

    test_probs = np.concatenate(test_probs, axis=0)

    # cacluate metrics
    y_true = test_y.reshape(-1)
    y_pred = test_probs.reshape(-1)

    mask = y_true != 0
    y_true_masked = y_true[mask] - 1  # so 2 -> 1, 1 -> 0
    y_pred_masked = y_pred[mask]

    return roc_auc_score(y_true_masked, y_pred_masked)


class ModelCompiler:
    def __init__(
            self,
            learning_rate,
            smoothing,
    ):

        self.loss_params = {
            'smoothing': smoothing,
            'learning_rate': learning_rate,
        }

    def get_compiler_params(self):
        lr = tf.keras.optimizers.schedules.ExponentialDecay(**self.optimizer_params)

        compile_params = {
            'optimizer': tf.keras.optimizers.Adam(learning_rate=lr),
            'loss': PaddedBinaryCrossentropyLoss(**self.loss_params)
        }

        return compile_params


def train_one_cv(
        dataset,
        data_dir,
        cv_idx,
        data_config,
        model_params,
        compiler_params,
        fit_params):
    logger.info(f'\n cv {cv_idx} starts for {dataset}\n')

    if dataset == 'synthetic5':
        max_len = 50
    else:
        max_len = dpfa_config.MAX_LEN

    data = preprocessing.DataSet(
        dataset,
        data_dir,
        max_len=max_len,
        cv_idx=cv_idx,
        **data_config)

    data.load_data()

    if cv_idx == 0:
        data.describe_data()  # only need to describe the data for the first time training

    (train_inputs, train_targets), (test_inputs, test_targets) = data.preprocess()

    model = dpfa_model.DPFABase(
        item_vocab_size=data.item_vocab_size,
        **model_params)
    logger.info(f'model params are {model.params}')
    logger.info('model is created')

    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=compiler_params['lr']),
        loss=PaddedBinaryCrossentropyLoss(compiler_params['smoothing']))
    logger.info('model is compiled')

    logger.info('start fitting')
    model.fit(
        train_inputs,
        train_targets,
        verbose=dpfa_config.VERBOSE,
        **fit_params)

    score = evaluate(
        test_inputs,
        test_targets,
        model,
        test_targets.shape[0],
        batch_size=128)
    # todo: change this level
    logger.info(f'test auc = {score} in cv = {cv_idx}')
    logger.info(f'\n cv {cv_idx} finished for {dataset}\n')

    return score


def train_cv(dataset, opt):
    data_config = dpfa_config.config[dataset]

    if dataset == 'assist2017':
        model_params, fit_params, compile_params = dpfa_config.assist_2017_params
    elif dataset == 'stat2011':
        model_params, fit_params, compile_params = dpfa_config.stat_2011_params
    elif dataset == 'synthetic5':
        model_params, fit_params, compile_params = dpfa_config.synthetic_5_params
    elif dataset == 'nips2020':
        model_params, fit_params, compile_params = dpfa_config.nips_2020_params
    else:
        raise NotImplementedError

    overwrite_default_args(model_params, opt)
    overwrite_default_args(fit_params, opt)
    overwrite_default_args(compile_params, opt)

    logger.info(f'fit params are {fit_params}')
    logger.info(f'optimizer and loss params = {compile_params}')

    cv_scores = []
    cv_start_idx = opt.cv_start_idx or 0
    cv_stop_dix = opt.cv_stop_idx or dpfa_config.CV_NUM

    for cv_idx in range(cv_start_idx, cv_stop_dix):
        tf.keras.backend.clear_session()
        logger.info('session cleared')
        cv_score = train_one_cv(
            dataset,
            (opt.dir or dpfa_config.DATA_DIR),
            cv_idx,
            data_config,
            model_params,
            compile_params,
            fit_params)
        cv_scores.append(cv_score)

    # todo: change this level
    logger.info(f'average test auc for {dataset} is {sum(cv_scores) / dpfa_config.CV_NUM}')
    return cv_scores


def get_parser():
    parser = ArgumentParser()

    # not all arguments are exposed to change for users
    # all arguments controlling the behavior of the model is in dpfa_config,
    # for complete control over the model, modify dpfa_config
    parser.add_argument(
        '--dataset',
        choices=['assist2017', 'stat2011', 'synthetic5', 'nips2020'],
        required=True,
        help='dataset to run the model'
    )

    parser.add_argument(
        '--dir',
        help='folder that hold data'
    )

    parser.add_argument(
        '--verbose',
        '-v',
        choices=[0, 1],
        type=int,
        help='model verbosity, set to 1 for more information; 0 to suppress information')

    # model args
    parser.add_argument(
        '--hidden_size',
        help='hidden size for item embedding',
        type=int,
    )

    parser.add_argument(
        '--dropout',
        help='dropout rate',
        type=float,
    )

    parser.add_argument(
        '--regulate_dot_product',
        help='force item correlation to large than 0',
        choices=[0, 1],
        type=int
    )

    parser.add_argument(
        '--time_decay',
        help='assign higher weight to recent items',
        choices=[0, 1],
        type=int
    )

    parser.add_argument(
        '--normalize_embedding',
        help='force item correlation to smaller than 1',
        choices=[0, 1],
        type=int
    )

    # fit args
    parser.add_argument(
        '--batch_size',
        type=int,
        help='training batch size'
    )

    parser.add_argument(
        '--epochs',
        type=int,
        help='number of epochs to train'
    )

    parser.add_argument(
        '--validation_split',
        type=float,
        help='poriton of training with hold for validation'
    )

    parser.add_argument(
        '--lr',
        type=float,
        help='learning rate'
    )

    parser.add_argument(
        '--cv_start_idx',
        type=int,
        help='cv start index'
    )

    parser.add_argument(
        '--cv_stop_idx',
        type=int,
        help='cv stop index'
    )

    return parser


def overwrite_default_args(default_config, opt):
    boolean_opts = ['regulate_dot_product', 'time_decay', 'normalize_embedding']
    for key in default_config:
        user_opt = getattr(opt, key, None)
        if user_opt is not None:
            if key in boolean_opts:
                user_opt = bool(user_opt)
            default_config[key] = user_opt


if __name__ == '__main__':
    parser = get_parser()
    opt = parser.parse_args()
    train_cv(opt.dataset, opt)




